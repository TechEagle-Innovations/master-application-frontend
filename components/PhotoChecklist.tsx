...
export interface PhotoChecklistProps {
  headerText: string;
  fetchChecklistItems: () => Promise<ChecklistItem[]>;
  submitChecklist: (items: ChecklistItem[]) => Promise<void>; // updated: no photos
  token?: string;
}
...
export default function PhotoChecklist({
  ...
  const toggleItemConfirm = (index: number) => {
    setChecklistItems(prev => {
      const updated = [...prev];
      updated[index].confirm = !updated[index].confirm;
      return updated;
    });
  };
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitLoading(true);
    try {
      await submitChecklist(checklistItems); // updated: only items
    } catch (err: any) {
      Alert.alert('Error', err?.message || 'Failed to submit checklist');
    } finally {
      setSubmitLoading(false);
    }
  }, [allItemsConfirmed, checklistItems, submitChecklist]);
    ...
    <View className="flex-1 bg-white">
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 32 }}>
        {checklistItems.map((item, index) => (
          <View
            ...
            className="flex-row items-center justify-between bg-white py-4 px-2 rounded-lg border-b border-gray-100"
          >
            <Text className="text-base text-gray-900 w-8/12">{item.label}</Text>
            <TouchableOpacity
              onPress={() => toggleItemConfirm(index)}
              className={`w-6 h-6 rounded-full border-2 ${item.confirm ? 'border-orange-500 bg-orange-500' : 'border-gray-400 bg-white'}`}
      ...
      <View style={{ padding: 16, backgroundColor: 'white', borderTopWidth: 1, borderColor: '#f3f4f6' }}>
        <TouchableOpacity
          ...
          accessibilityLabel="Submit checklist"
          onPress={handleSubmit}
          disabled={!allItemsConfirmed || submitLoading}
          style={{ opacity: allItemsConfirmed ? 1 : 0.5 }}
        >
          {submitLoading ? (
            <ActivityIndicator color="white" />
          ) : (
